...
![Tab Summarizer Screenshot](screenshot.png)

## How It Works

1. Click the Tab Summarizer icon in your browser toolbar.
2. Click **"Summarize this page"**.
3. Get a 3-point summary in seconds.

The extension extracts the page text, sends it to a Vercel serverless function, which forwards it to the Groq API and returns a summary.
...
Open `extension/popup.js` and replace the fetch URL with your own Vercel URL:
```javascript
...
> **Tip:** Works best on articles, blog posts, and news pages. May not work on pages that block content scripts such as browser settings pages.